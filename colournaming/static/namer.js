...
function hexToRGB(hexcode) {
  return [parseInt(hexcode.substring(0, 2), 16), parseInt(hexcode.substring(2, 4), 16), parseInt(hexcode.substring(4, 6), 16)];
}
...
function doQuery(hexcode, responseHandler) {
  ...
  $.get(COLOUR_NAMER_URL + $languageSelect.val() + '/name?colour=' + hexcodeValues, function (data) {
    ...
    (0, _audio.playSound)(topHex);
    var $synonyms = $('<ul/>');
    $.each(data.colours, function (i, colour) {
      var $li = $('<li/>').text(colour.name.replace('_', ' '));
      $synonyms.append($li);
    });
...
function resetSelectName(hexCode) {
  ...
  if (hexCode === undefined || hexCode.slice(1) !== $nameSelect.val()) {
    $nameSelect.blur().val('-').children(':first-child').attr('selected', 'selected');
  }
}

function resetAgreement() {
  console.log('in agreement reset');
  var $agreementSelect = $('select#agreement :first-child')[0];
  $agreementSelect.selected = true;
  /*console.log('in agreement reset');
  console.log($('select#agreement'));
  console.log($('select#agreement').children);
  $('select#agreement').children[0].selected = true;
  $agreementSelect.blur();*/
}
...
function updateNames() {
  ...
  $.get(COLOUR_NAMER_COLOURS_URL + '?lang=' + $languageSelect.val(), function (data) {
    ...
    $.each(data, function (_, colour) {
      $nameSelect.append($('<option/>').attr('value', colour.hex).text(colour.name));
    });
  });
}
...
function updateLanguages() {
  ...
  $.get(COLOUR_NAMER_LANGUAGES_URL, function (data) {
    ...
    $.each(data, function (_, language) {
      $languageSelect.append($('<option/>').attr('value', language.code).text(language.name));
    });
  });
}
...
(function ($) {
  ...
  $._farbtastic = function (container, callback) {
    ...
    fb.linkTo = function (callback) {
      ...
      } else if ((typeof callback === 'undefined' ? 'undefined' : _typeof(callback)) == 'object' || typeof callback == 'string') {
        fb.callback = $(callback);
