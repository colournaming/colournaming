...
def save_participant(experiment):
    ...
    if participant_id is None:
        participant = MturkAgeParticipant(
...
def save_response(experiment, response):
    ...
    participant = MturkAgeParticipant.query.filter(
        MturkAgeParticipant.id == experiment["participant_id"]
    ).one()
    colour_response = MturkAgeColourResponse(
    ...
    db.session.commit()
    return MturkAgeColourResponse.query.filter(
        MturkAgeColourResponse.participant == participant
    ).count()
